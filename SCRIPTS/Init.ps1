...
if ($Global:gsGlobalSettingsSuccessfullyLoaded -or (-not $InitGlobal )) {
    ...
    $Global:gsScriptBaseFileName  = $gsScriptFileName.split(".")[0]
    $Global:gsScriptLogFilePath   = "$ProjectRoot\$($Global:gsLOGSFolder)\$ScriptFileName.log"
    $Global:gsScriptArguments     = $Global:gsScriptArguments.Trim()
    $Global:gsParentLevel         = $Global:ScriptStack.count
    ...
    Add-ToLog -Message "Script [$ScriptWithArgs] with PID [$($PID)] started under [$($RunningCredentials.Name)]." -logFilePath $Global:gsScriptLogFilePath -Display -Status "Info" -Level $Global:gsParentLevel
}
exit 0
